...
class LogController extends Controller
{
	...
	public function accessRules()
	{
		return array(
			array('allow',  // allow all users to perform 'index' and 'view' actions
				'actions'=>array('index','view','inbound'),
				'users'=>array('@'),
			),
	...
	public function actionIndex($controller=null)
	{
        ...
        if(!is_null($controller))$model->controller=$controller;    //default filter
        if(isset($_GET['Log']))
    ...
    public function actionInbound($oid,$a=20,$b=3){
        $log = '';
        ...
        /*if($org!=NULL)*/{
            ...
            $condition = '\"'.$org->name;
            //$cmd = "cat $log_full | grep \"$condition\" -B $b -A $a > /tmp/$org->name";
            $cmd = "/bin/cat $log_full | grep \"$condition\" -B $b -A $a";
            //$cmd = "$log_full | grep \"$condition\" -B $b -A $a";
            $log = `$cmd`;
            //$log = file_get_contents("/tmp/$org->name");
            //print_r($cmd);
            //print_r($log);
        }
